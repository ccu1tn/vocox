...
# from tensorflow.python.keras.layers import Conv2D, Conv2DTranspose, GlobalAveragePooling2D, Concatenate
...
class Chan_Model(object):  
    ...
    def __init__(self, name, pop_size, elite_size, noise_params):
        self.name = name
    ...
    def create_population(self, population_size):
        return [(random.uniform(0, 1), random.uniform(0, 10), random.uniform(0, 2 * np.pi)) for _ in range(population_size)]
    
    def genetic_generation(self, population_size, generations, mutation_rate, signal_length):
        best_params = genetic_algorithm(population_size, generations, mutation_rate, signal_length)
        return best_params
    ...
    def __call__(self, _input, std):
        ...
        # Genetic algorithm to generate noise parameters
        # length_value = 2
        # noise_params = self.genetic_generation(population_size=10, generations=5, mutation_rate=0.1, signal_length=100)
        # # print('length',tf.shape(x_complex)[2])
        # # print('length',x_complex)
        # # exit()
        # print('noise_params',noise_params)
        
        # shape_tensor = tf.shape(noise_params)
        # print('shape_tensor',shape_tensor)
        
        # # Convert the shape tensor to a NumPy array
        # #noise_length = shape_tensor.numpy()
        # noise_length = int(shape_tensor)
        # #noise_length = tf.shape(x_complex).numpy()
        # # 
        # print('noise_length',noise_length)
        
        # Generate noise using the best parameters
        #noise = generate_noise(*noise_params, length=tf.shape(x_complex)[2])
        #length_value = tf.shape(x_complex)[2]
        length_value = 2
